...
import { UnitConversionField, type UnitOption } from "./unit-conversion-field";
...
export function CreateProductForm({
  ...
  const form = useForm<CreateProductFormValues>({
    ...
    defaultValues: {
      ...
      units: product?.units ?? [
        { unitId: units[0]?.id ?? "", conversionFactor: 1, isDefault: true, sellingPrice: 0, purchasePrice: 0 },
      ],
  ...
  const baseUnit = useMemo(() => {
    const defaultRow = watchedUnits.find((u) => u.isDefault);
    return defaultRow?.unitId ? unitsById.get(defaultRow.unitId) : undefined;
  }, [watchedUnits, unitsById]);
  ...
  async function upload(file: File) {
    ...
    form.setValue("imageUrl", supabase.storage.from(BUCKET).getPublicUrl(path).data.publicUrl);
  ...
  async function onSubmit(values: CreateProductFormValues) {
    const result =
      mode === "edit" && product
        ? await updateProduct(workspaceId, workspaceSlug, product.id, values)
        : await createProduct(workspaceId, workspaceSlug, values);
    if (result.success) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-7">
        <section className="grid gap-4 md:grid-cols-2">
          <h2 className="md:col-span-2 text-lg font-semibold">Basic information</h2>
          <FormField
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value || ""} placeholder="SHIRT-001" disabled={submitting} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value || ""} placeholder="Or type a new one" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="md:col-span-2">
                ...
                <FormControl>
                  <Input {...field} value={field.value || ""} placeholder="Optional product notes" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
              ...
              <FormItem className="md:col-span-2">
                ...
                <FormControl>
                  <div className="flex gap-4 rounded-lg border p-4">
                    {imageUrl ? (
                      <div className="h-16 w-16 rounded bg-cover bg-center" style={{ backgroundImage: `url(${imageUrl})` }} />
                    ) : (
                    ...
                    <div>
                      ...
                      {imageUrl && (
                        <Button type="button" variant="ghost" onClick={() => form.setValue("imageUrl", undefined)}>
                          Remove
                        </Button>
                      )}
            ...
            <Button
              ...
              onClick={() =>
                fields.append({
                  unitId: "",
                  conversionFactor: 1,
                  isDefault: fields.fields.length === 0,
                  sellingPrice: 0,
                  purchasePrice: 0,
                })
              }
