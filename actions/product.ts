...
function getStoragePath(publicUrl: string | null) {
  ...
  try {
    ...
    return bucketIndex === -1
      ? null
      : decodeURIComponent(parts.slice(bucketIndex + 1).join("/"));
  } catch {
    return null;
  }
}
...
async function replaceProductUnits(
  supabase: Awaited<ReturnType<typeof createClient>>,
  productId: string,
  values: CreateProductFormValues,
) {
  const { data: currentUnits, error: currentError } = await supabase
    .from("product_units")
    .select("id")
  ...
  const { data: units, error: unitsError } = await supabase
    ...
    .select("id, unit_id");
  if (unitsError || !units)
    throw new Error(unitsError?.message ?? "Could not save product units");
...
export async function createProduct(
  ...
  try {
    if (
      ...
      return {
        message: `SKU "${values.sku}" is already used by another product.`,
        success: false,
      };
    if (
      values.barcode &&
      ...
      return {
        message: `Barcode "${values.barcode}" is already used by another product.`,
        success: false,
      };
...
export async function updateProduct(
  ...
  if (existing.image_url && existing.image_url !== values.imageUrl) {
    const path = getStoragePath(existing.image_url);
